...
console.log(chalk.bold.italic.magenta(`\t\t    WELCOME TO CODE WITH RIMSHA TODO LIST APPLICATION     `));
console.log(chalk.red("\t\t>>>>>>>/==============================================/<<<<<<<<<\n\t\t"));
let todos = [];
let condition = true;
while (condition) {
    ...
    if (!addTask.add) {
        ...
        console.log(chalk.italic.cyanBright(addTask.add, "\tIs Added In Todo List Successfully"));
        ...
        if ((condition = addMoreTask.addMore)) {
            ...
            if ((condition = confirmAns.confirmation)) {
                ...
                if (answer.operation == "delete") {
                    ...
                    let newtodo = todos.filter((val) => val !== deleteAns.remove);
                    todos = [...newtodo];
